...
class Game():
    def __init__(self, hero, lvl, position):
        ...
        self.field = [[random.choice(["пусто", "сундук", "монстр", "ловушка"]) for _ in range(self.field_size)] for _ in
                      range(self.field_size)]  # создается игровое поле со случайнвми комнатами

        key_x, key_y = random.randint(0, self.field_size - 1), random.randint(0,
                                                                              self.field_size - 1)  # вставляем ключ на какую то позициб
    ...
    # РЕЖИМ РАЗРАБОТЧИКА - ПОКАЗАТЬ КАРТУ С ПРЕДМЕТАМИ (УДАЛИТЬ ПОСЛЕ ТЕСТИРОВАНИЯ)
    def show_dev_map(self):
        print("\n=== РЕЖИМ РАЗРАБОТЧИКА ===")
        print("К - ключ, П - портал, М - монстр, С - сундук, Л - ловушка, _ - пусто")
        print("Игрок находится на позиции:", (self.position[0] + 1, self.position[1] + 1))
    ...
    def play(self):
        ...
        while not self.game_over:
            ...
            # РЕЖИМ РАЗРАБОТЧИКА - ПОКАЗАТЬ КАРТУ (УДАЛИТЬ ПОСЛЕ ТЕСТИРОВАНИЯ)
            self.show_dev_map()
